...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Fixed Sparkline Mapping Calculations
    ...
    const kpi = {
      ...
      issuesIdentified: {
        ...
        delta:
          latestAnalysis && prevAnalysis
            ? (latestAnalysis.issues?.length || 0) - (prevAnalysis.issues?.length || 0)
            : null,
        spark: issuesSpark,
      },
      keywordsMatched: {
        ...
        total: latestAnalysis
          ? (latestAnalysis.keywordsPresent?.length || 0) + (latestAnalysis.keywordsMissing?.length || 0)
          : null,
        delta:
          latestAnalysis && prevAnalysis
            ? (latestAnalysis.keywordsPresent?.length || 0) - (prevAnalysis.keywordsPresent?.length || 0)
            : null,
        spark: keywordsSpark,
      },
    };
